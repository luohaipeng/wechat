...
<script type="text/javascript">
    ...
    function clickCatalog(event, treeId, treeNode) {
        $("#propertyList").load("/catalogs/"+treeNode.id+"/skuPropertys");
    }
    ...
    function showPropertyValue(id) {
        ...
        $("#propertyValueForm").load("skuPropertys/"+id+"/skuPropertyValues")
    }

    //刷新属性列表
    function refreshPropertyList() {
        var nodes = treeObj.getSelectedNodes();
        $("#propertyList").load("/catalogs/"+nodes[0].id+"/skuPropertys");
    }
    ...
    function deletePropertyValue(obj) {
        ...
        $.messager.confirm("提示","是否确定删除",function () {
            //如果id不为0，那就发送请求到服务器删除该属性值
            if(id){
                $.ajax({
                    url:"/skuPropertyValues/"+id,
                    type:"DELETE",
                    success:function (data) {
                        if(data.success){
                            $.messager.popup("删除成功");
    ...
    <div class="modal fade" id="myModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="/skuPropertys" method="post" id="propertyForm">
